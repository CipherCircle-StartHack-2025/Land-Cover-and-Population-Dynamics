...
def get_chatbot_response(user_message):
    ...
    system_prompt = (
        ...
        "Ensure your responses are concise, friendly, and remain within the context of the Assaba Region EDA Dashboard. "
        "Do not provide external advice beyond what is shown in the dashboard data. "
        "Follow all standard safety guardrails."
    )
    try:
...
muni_gdf = gpd.read_file("/Users/timurtaepov/Desktop/eda_Data/Admin_layers/Assaba_Districts_layer.shp")
...
gpp_choro_dict = {}
gpp_folder = "/Users/timurtaepov/Desktop/eda_Data/MODIS_Gross_Primary_Production_GPP/"
...
###############################################################################
landcover_data_folder = "/Users/timurtaepov/Desktop/eda_Data/Modis_Land_Cover_Data"
...
gpp_data_dir = "/Users/timurtaepov/Desktop/eda_Data/MODIS_Gross_Primary_Production_GPP"
precip_data_dir = "/Users/timurtaepov/Desktop/eda_Data/Climate_Precipitation_Data"
...
language_dict = {
    "EN": {
        ...
        "compare": "Compare",
        "additional_info": load_text_markdown("english_report.txt")
    },
    "FR": {
        "header": "Addax",
        ...
        "compare": "Comparer",
        "additional_info": load_text_markdown("french_report.txt")
    },
    "AR": {
        "header": "أداكس",
        ...
        "compare": "مقارنة",
        "additional_info": load_text_markdown("arabic_report.txt")
    }
}
...
app.layout = dbc.Container([
    ...
    dbc.Row([
        dbc.Col(
            html.H1(id="header-text", className="text-center text-primary mb-4"),
            width=10
        ),
        dbc.Col(
            dcc.Dropdown(
            ...
            dcc.Tabs([
                ...
                dcc.Tab(label=language_dict["EN"]["muni_comparison"], children=[
                    html.Div([
                        ...
                        html.Button(language_dict["EN"]["compare"], id='compare-button', n_clicks=0, 
                                    className="btn btn-primary", style={'margin': '10px'}),
                        dcc.Graph(id='municipality-comparison-graph', className="graph-container")
                    ])
                ])
            ], style={"backgroundColor": "#ffffff", "padding": "10px"}),
        ], width=6, style={"backgroundColor": "#f7f7f7", "padding": "10px"}),

        # Right column: Additional info + Chatbot
        dbc.Col([
            # Additional Info in a card
            dbc.Card([
                dbc.CardHeader(html.H4(id="additional-info-title")),
                dbc.CardBody(
                    dcc.Markdown(
                        id="additional-info-text",
                        style={
            "fontFamily": "Arial, sans-serif",
            "maxHeight": "400px",   # Maximum height for the content area
            "overflowY": "auto",    # Enable vertical scrolling when needed
            "overflowX": "hidden",  # Disable horizontal scrolling
            "whiteSpace": "normal", # Wrap text normally instead of preserving newlines
            "wordBreak": "break-all"  # Break long words if needed to avoid horizontal scroll
        }
                    )
                )
            ], className="mb-3", style={"backgroundColor": "#ffffff", "border": "none"}),

            # Chatbot in a card
            dbc.Card([
                dbc.CardHeader("Dashboard Chatbot", style={"fontWeight": "bold"}),
                dbc.CardBody([
                    html.Div(
                        id='chat-history',
                        children=[],
                    ...
                    html.Br(),
                    dbc.InputGroup([
                        dbc.Input(id='chat-input', placeholder='Enter your message...', type='text'),
                        dbc.Button("Ask", id='chat-send', n_clicks=0, color="primary")
                    ])
                ], style={"marginBottom": "50px"})  # Extra bottom margin for the chatbot
            ], style={"backgroundColor": "#ffffff", "border": "none"})
        ], width=6, style={"backgroundColor": "#f5f5f5", "padding": "10px"})
    ])
],
fluid=True,
style={"backgroundColor": "#eaeaea", "minHeight": "100vh", "paddingBottom": "100px"}
)
